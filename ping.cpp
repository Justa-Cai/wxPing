...
#ifdef CPING_USE_ICMP
BOOL CPing::Ping1(const char* pHostName, CPingReply& pr,  funcPingCallBack pFunc, int times, UCHAR nTTL, DWORD dwTimeout, UCHAR nPacketSize) const
{
	int i;
	//For correct operation of the T2A macro, see TN059
	//USES_CONVERSION;
	...
	unsigned long	addr = inet_addr(pHostName);
	if (addr == INADDR_NONE)
	{
		//Not a dotted address, then do a lookup of the name
		hostent* hp = gethostbyname(pHostName);
		if (hp)
